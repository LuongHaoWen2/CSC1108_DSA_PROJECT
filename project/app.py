...
def index():
    ...
    if request.method == "POST":
        ...
        if selected_origin and selected_origin in data:
            ...
            # If a destination is selected, populate carriers
            if selected_destination:
                for route in data[selected_origin].get("routes", []):
                    if route["iata"] == selected_destination:
                        carrier_options = [(c["iata"], c["name"]) for c in route.get("carriers", [])]

        # Only allow route info if everything is valid
        if selected_origin and selected_destination and selected_destination in [c for c, _ in destination_options]:
            carrier_text = f" with carrier {selected_carrier}" if selected_carrier else ""
            route_info = f"Route from {selected_origin} to {selected_destination}{carrier_text} using '{route_preference}' preference."
