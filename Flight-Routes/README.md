...
└── static/               # Static assets
    └── js/
        └── globe.js     # Map initialization (legacy)
```
...
- `max_stops`: Maximum number of stops (integer or "unlimited")
...
1. For new algorithms: Create a new file in `algo/` and export a search function
2. For new API endpoints: Add to corresponding list in `routes/api.py`
3. For UI changes: Modify `templates/index.html` and link new elements to JavaScript handlers

### Running Tests

Currently no automated tests. Manual testing through web interface is primary method.

### Version Control

**Important**: Do NOT commit the `venv/` folder to Git. The `.gitignore` file already excludes it. Other developers can recreate the environment using:
```
python -m venv venv
.\venv\Scripts\Activate.ps1
pip install -r requirements.txt
...
Educational project for CSC1108 - Data Structures and Algorithms

## Author

Created as part of Year 1, Trimester 2 coursework
